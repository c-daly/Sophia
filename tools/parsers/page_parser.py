...
class PageParser(ABC):
    ...
    def calculate_document_score(self, chunk: str, context: Dict[str, Any]) -> float:
        ...
        """
        # Simple heuristic: longer chunks and chunks from important sections get higher scores
        base_score = min(len(chunk) / 500.0, 1.0)  # Longer chunks up to 500 chars
        
        # Boost score based on HTML context
        if context.get('tag') in ['h1', 'h2', 'h3', 'title']:
            base_score *= 1.5
        elif context.get('tag') in ['p', 'div']:
            base_score *= 1.0
        elif context.get('tag') in ['span', 'small']:
            base_score *= 0.8
    ...
    def calculate_query_score(self, chunk: str, query: str) -> float:
        """
        Calculate how relevant a chunk is to a specific query.
        ...
        # Calculate partial matches (simple substring approach)
        partial_matches = sum(1 for word in query_words 
                            if any(word[:3] in chunk_word for chunk_word in chunk_lower.split())
                            and word not in chunk_lower)
        
        # Score based on match ratio
        total_words = len(query_words)
        score = (exact_matches + partial_matches * 0.5) / total_words if total_words > 0 else 0.0
