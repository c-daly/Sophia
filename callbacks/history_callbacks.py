...
def toggle_modal(active_cell, close_clicks, next_clicks, prev_clicks, table_data):
    ...
    if trigger_id == "history-datatable" and active_cell:
        ...
        config.logger.debug(
            f"toggle_modal called with active_cell: {active_cell}, close_clicks: {close_clicks}, next_clicks: {next_clicks}, prev_clicks: {prev_clicks}, messages: {messages}")
...
def update_table(search_query, n_clicks):
   ...
   #tooltip_data = [
    ...
    #    for doc in data
    #]
    config.logger.debug(f"Data[0]: {data[0]}")
    config.logger.debug(f"Data[1]: {data[1]}")
    return [data]
    #return dff.iloc[
