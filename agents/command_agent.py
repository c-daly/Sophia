...
class CommandAgent(AbstractAgent):
    ...
    def save_interaction_to_database(self):
       ...
       #summary = summary.choices[0].message['content']
        ...
        self.logger.debug(f"Summary embedding: {summary_embedding}")
        #self.messages = []
        self.messages.append({"role": "assistant", "content": summary})
        #self.messages.append({"role": "system", "content": f"You have recollection of having had the following conversations with the user: {self.summaries}"})
