...
class MemoryEquippedAgent(BasicAgent):
    ...
    def apply_tools(self, text):
        ...
        #interim_result = [f"{{'title': '{result.title}', 'description': '{result.description}', 'url': '{result.url}'}}"for result in search_result]
        interim_result = [{ 'title': result.title, 'description': result.description, 'url': result.url } for result in search_result]
        config.logger.info(f"Google Search Results: {interim_result}")
        interim_json = json.dumps(interim_result)
